...
> 图片识别 (Qwen-VL) + 联网搜索 (Tavily)，支持 **stdio** 和 **HTTP/SSE** 双传输层。
> 适配: Claude Code / Codex / OpenCode / Reasonix 及任何支持 MCP 的 agent。

## 快速开始

```bash
git clone https://github.com/x12315/codex-mcp-migration.git
...
```bash
uv run mcp-hub stdio vision
uv run mcp-hub stdio search
```
...
```bash
uv run mcp-hub serve vision --port 8080
uv run mcp-hub serve search --port 8081
```
...
也可通过环境变量 `MCP_HUB_ENV_FILE` 指定自定义路径。
...
```bash
# alpha (Arch Linux) 上
git clone https://github.com/x12315/codex-mcp-migration.git ~/codex-mcp-migration
cd ~/codex-mcp-migration
uv sync
